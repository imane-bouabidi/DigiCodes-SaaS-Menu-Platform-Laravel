...
<form action="{{ route('menu_items.store') }}" method="POST" class="h-full w-full p-4 flex flex-col md:flex-row justify-between gap-4">
    @csrf
    <div class="w-full p-4 bg-white rounded-sm transition-shadow box-border color-opacity-87   shadow-md backdrop-blur-md">
        <h3 class="mb-4 text-3xl font-semibold text-gray-500">Add new Menu Item</h3>
        ...
        <label for="price" class="flex flex-col gap-y-1 mt-8">
            Price
            <input type="number" id="price" name="price" class="rounded-sm border-md border-gray-200 forced-colors:text-blue-600" placeholder="Enter price">
        </label>
        <label for="menu_id" class="flex flex-col gap-y-1 mt-8">
            Menu
            <select id="menu_id" name="menu_id" class="rounded-sm border-md border-gray-200 forced-colors:text-blue-600">
